$(document).ready(function () {
    ...
    /********************** Toggle Map Content (per column) **********************/
    $('.map-column-wrap').each(function () {
        var $col = $(this);
        $col.find('.btn-show-map').on('click', function () {
            $col.find('.map-content-panel').toggleClass('toggle-map-content');
            $col.find('.btn-show-content').toggleClass('toggle-map-content');
        });
        $col.find('.btn-show-content').on('click', function () {
            $col.find('.map-content-panel').toggleClass('toggle-map-content');
            $col.find('.btn-show-content').toggleClass('toggle-map-content');
        });
    });
    ...
    $('#rsvp-form').on('submit', function (e) {
        ...
        if (MD5($('#invite_code').val()) !== 'b0e53b10c1f55ede516b240036b88f40'
            && MD5($('#invite_code').val()) !== '29223515026893958260511d3e13da44') {
            $('#alert-wrapper').html(alert_markup('danger', '<strong>Sorry!</strong> Your invite code is incorrect.'));
        } else {
            $.post('https://script.google.com/macros/s/AKfycbxgfzwty2I1VlsgWED0r-FChf_rEPx-ZzZbOyutd35TIYyfXLDTz1W2Y57KpAc61Ahf/exec', data)
                .done(function (data) {
                    console.log(data);
...
// Google map (two columns: church + reception)
function initMap() {
    var churchLoc = {lat: 7.459869212540602, lng: 125.81294205687611};
    var receptionLoc = {lat: 7.4273606475989284, lng: 125.78657930903032};

    var churchEl = document.getElementById('map-canvas-church');
    var receptionEl = document.getElementById('map-canvas-reception');
    if (!churchEl || !receptionEl) {
        return;
    }

    var churchMap = new google.maps.Map(churchEl, {
        zoom: 17,
        center: churchLoc,
        scrollwheel: true,
        mapTypeId: 'satellite'
    ...
    var receptionMap = new google.maps.Map(receptionEl, {
        zoom: 17,
        center: receptionLoc,
        scrollwheel: true,
        mapTypeId: 'satellite'
...
function initBBSRMap() {
    ...
    var el = document.getElementById('map-canvas-reception');
    if (!el) {
        return;
    }
